...
- Da them `audio_replacement_service` de build va chay FFmpeg command thay audio toan phan vao `{job}/working/{videoId}_audio_replaced.mp4`.
- Da them policy check `ReplaceAll` / `NoReplacement`; neu policy khong cho thay audio thi skip step, khong goi FFmpeg.
- Da cap nhat `job_orchestrator` de co lap loi per-video, danh dau `Failed` cho video loi, va tiep tuc queue voi video ke tiep.
- Da ghi log theo tung video tai `{job}/logs/{videoId}.log` va emit `audioReplacementStarted` / `audioReplacementCompleted`.

### File List

- `src-tauri/src/services/audio_replacement_service.rs`
